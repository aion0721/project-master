...
function buildLevels(
  systemIds: string[],
  edges: Array<{ sourceSystemId: string; targetSystemId: string }>,
) {
  ...
  edges.forEach((edge) => {
    outgoing.set(edge.sourceSystemId, [
      ...(outgoing.get(edge.sourceSystemId) ?? []),
      edge.targetSystemId,
    ]);
    indegree.set(
      edge.targetSystemId,
      (indegree.get(edge.targetSystemId) ?? 0) + 1,
    );
  });
  ...
  while (queue.length > 0) {
    ...
    for (const next of outgoing.get(current) ?? []) {
      level.set(
        next,
        Math.max(level.get(next) ?? 0, (level.get(current) ?? 0) + 1),
      );
...
export function SystemLandscapePage() {
  const { systems, systemRelations, projects, isLoading, error } =
    useProjectData();
  ...
  const diagram = useMemo(() => {
    ...
    for (const [columnIndex, columnSystems] of [...columns.entries()].sort(
      (a, b) => a[0] - b[0],
    )) {
    ...
    return {
      ...
      width:
        canvasPadding * 2 + (maxColumn + 1) * nodeWidth + maxColumn * columnGap,
      height:
        canvasPadding * 2 +
        Math.max(maxRows, 1) * nodeHeight +
        Math.max(maxRows - 1, 0) * rowGap,
    };
        ...
        {selectedSystem && focusedView ? (
          <div className={styles.focusGrid}>
                ...
                {focusedView.upstream.length > 0 ? (
                  focusedView.upstream.map(({ relation, system }) => (
                    <article className={styles.systemCard} key={relation.id}>
                      <strong className={styles.systemCardTitle}>
                        {system.name}
                      </strong>
                      ...
                      {relation.note ? (
                        <p className={styles.systemCardNote}>{relation.note}</p>
                      ) : null}
                    </article>
                  ))
                ) : (
                  <p className={styles.emptyText}>
                    このシステムにデータを渡す上流システムはありません。
                  </p>
                )}
              </div>
            </section>
            ...
            >
              ...
              <article
                className={`${styles.systemCard} ${styles.systemCardPrimary}`}
              ...
              >
                ...
                <span className={styles.systemCardMeta}>
                  関連案件 {projectCountBySystemId.get(selectedSystem.id) ?? 0}{" "}
                  件
                </span>
                {selectedSystem.note ? (
                  <p className={styles.systemCardNote}>{selectedSystem.note}</p>
                ) : null}
              </article>
            </section>
              ...
              {diagram.edges.map(({ relation, source, target }) => {
                ...
                const hideEdgeTooltip = () =>
                  setHoveredEdge((current) =>
                    current?.id === relation.id ? null : current,
                  );
                ...
                return (
                  <g key={relation.id}>
                    ...
                    />
                    <text
                      className={`${styles.edgeLabel} ${isEdgeActive ? styles.edgeLabelActive : ""}`}
                      x={labelX}
                      y={labelY}
                    >
                      仕向け → 被仕向け
