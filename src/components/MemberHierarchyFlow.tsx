...
function collectMemberIds(node: HierarchyTreeNode): string[] {
  return [node.member.id, ...node.children.flatMap((child) => collectMemberIds(child))]
}
...
function flattenTree(
  ...
  node.children.forEach((child) => {
    ...
    const branchColor = branchTone === undefined ? null : branchTones[branchTone]
    ...
    edges.push({
      ...
      markerEnd: {
        ...
        width: 16,
        height: 16,
        color: child.isSelected ? '#0f766e' : (branchColor?.stroke ?? '#a8b2c2'),
      },
      style: {
        stroke: child.isSelected ? '#0f766e' : (branchColor?.stroke ?? '#a8b2c2'),
...
function layoutNodes(nodes: Node<HierarchyNodeData>[], edges: Edge[]) {
...
function arrangeTopLevelBranches(nodes: Node<HierarchyNodeData>[], groups: BranchGroup[]) {
...
function buildBranchBandNodes(nodes: Node<HierarchyNodeData>[], groups: BranchGroup[]): Node<BranchBandData>[] {
  const nodeMap = new Map(nodes.map((node) => [node.id, node]))
  const rawBounds = groups
    .map((group) => {
      const groupNodes = group.memberIds
        .map((memberId) => nodeMap.get(memberId))
        .filter((node): node is Node<HierarchyNodeData> => Boolean(node))
      ...
      return {
        ...
        minX: Math.min(...groupNodes.map((node) => node.position.x)) - branchPaddingX,
        maxX: Math.max(...groupNodes.map((node) => node.position.x + nodeWidth)) + branchPaddingX,
        minY: Math.min(...groupNodes.map((node) => node.position.y)),
        maxY: Math.max(...groupNodes.map((node) => node.position.y + nodeHeight)),
      }
    })
    .filter(
      (
        item,
      ): item is { group: BranchGroup; nodeCount: number; minX: number; maxX: number; minY: number; maxY: number } =>
        Boolean(item),
    )
  ...
  const boundsByGroupId = new Map(adjustedBounds.map((bound) => [bound.group.id, bound]))
    ...
    return [
      {
        ...
        style: {
          width: Math.max(bound.maxX - bound.minX, nodeWidth + branchPaddingX * 2),
...
function MemberFlowNode({ data }: NodeProps<HierarchyNodeData>) {
  const className = [
    styles.nodeCard,
    ...
    <div className={className}>
      ...
      <div className={styles.nodeInner}>
        <div className={styles.nodeHeader}>
          {data.isSelected ? <span className={styles.stateBadge}>選択中</span> : null}
          {!data.isSelected && data.isPathNode ? <span className={styles.pathBadge}>Path</span> : null}
        </div>
...
function BranchBandNode({ data }: NodeProps<BranchBandData>) {
